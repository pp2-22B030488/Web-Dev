export interface Product  {
    name: string;
    description: string;
    link: string;
    imgURL: string;
    rating: number;
    url: string;
    numberOfLikes: number;
}
export const products=[{
    name:'Apple iPhone 13 128Gb черный',
    description:'Apple iPhone 13 получил дисплей 6.1 дюйма Super Retina XDR, который отличается невероятно высокой плотностью пикселей — фотографии, видео и текст выглядят поразительно четко. ',
    link:"https://kaspi.kz/shop/p/apple-iphone-13-128gb-chernyi-102298404/?c=750000000",
    imgURL:'https://resources.cdn-kaspi.kz/img/m/p/h32/h70/84378448199710.jpg?format=preview-large',
    rating:4.9,
    url:"https://t.me/zhakoooooooo12345/13",
    numberOfLikes:0
    },
    {
        name:'Xiaomi Redmi Note 10 Pro 8 ГБ/256 ГБ серый',
        description:'технология NFC: Да\nцвет: серый\nтип экрана: цветной AMOLED, сенсорный',
        link:"https://kaspi.kz/shop/p/xiaomi-redmi-note-10-pro-8-gb-256-gb-seryi-107221005/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/medias/sys_master/images/images/h30/hd2/64487157760030/xiaomi-redmi-note-10-pro-8-gb-128-gb-seryi-107221005-1.jpg',
        rating:4.4,
        url:"https://t.me/zhakoooooooo12345/14",
        numberOfLikes:0
    }
    ,
    {
        name:'Samsung Galaxy A13 4 ГБ/128 ГБ черный',
        description:'Благодаря 6.6-дюймовому экрану Infinity-V на смартфоне Galaxy A13 вы сможете увидеть все, что было скрыто от вашего взгляда.',
        link:"https://kaspi.kz/shop/p/samsung-galaxy-a13-4-gb-128-gb-chernyi-104253279/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/h1c/hac/64381277929502.jpg?format=preview-large',
        rating:4.7,
        url:"https://t.me/zhakoooooooo12345/15",
        numberOfLikes:0
    }
    ,
    {
        name:'Samsung Galaxy A23 6 ГБ/128 ГБ черный',
        description:'Откройте для себя больше возможностей. Получите всё и сразу. 6.6-дюймовый TFT-дисплей Galaxy A23 с V-образным вырезом позволяет видеть и делать больше. ',
        link:"https://kaspi.kz/shop/p/samsung-galaxy-a23-6-gb-128-gb-chernyi-104348541/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/had/hfc/64382158766110.jpg?format=gallery-medium',
        rating:4.4,
        url:"https://t.me/zhakoooooooo12345/17",
        numberOfLikes:0
    }
    ,
    {
        name:'Xiaomi Redmi Note 10 Pro 8 ГБ/256 ГБ серый',
        description:'технология NFC Откройте для себя больше возможностей',
        link:"https://kaspi.kz/shop/p/xiaomi-redmi-note-10-pro-8-gb-256-gb-seryi-107221005/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/medias/sys_master/images/images/h30/hd2/64487157760030/xiaomi-redmi-note-10-pro-8-gb-128-gb-seryi-107221005-1.jpg',
        rating:4.8,
        url:"https://t.me/zhakoooooooo12345/18",
        numberOfLikes:0
    }
    ,
    {
        name:'Samsung Galaxy A33 5G 6 ГБ/128 ГБ черный',
        description:'Смартфон Galaxy A33 5G стал тоньше и элегантнее благодаря матовому покрытию, качественным рамкам и безупречному корпусу камеры с Ambient Edge.',
        link:"https://kaspi.kz/shop/p/samsung-galaxy-a33-5g-6-gb-128-gb-chernyi-104398547/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/medias/sys_master/images/images/h98/h3f/49894748028958/samsung-galaxy-a33-5g-6-gb-128-gb-cernyj-104398547-1.jpg',
        rating:4.9,
        url:"https://t.me/zhakoooooooo12345/19",
        numberOfLikes:0
    }
    ,
    {
        name:'Apple iPhone 14 Pro 256Gb фиолетовый',
        description:' Модель оснащена дисплеем Super Retina XDR, пиковая яркость которого составляет 2000 кд/м². Дисплей также имеет частоту обновления 120 Гц с технологией LTPO.',
        link:"https://kaspi.kz/shop/p/apple-iphone-14-pro-256gb-fioletovyi-106363319/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/h7b/hdc/64428132991006.jpg?format=preview-medium',
        rating:5,
        url:"https://t.me/zhakoooooooo12345/20",
        numberOfLikes:0
    }
    ,
    {
        name:'Apple iPhone 14 128Gb голубой',
        description:'Модель оснащена дисплеем Super Retina XDR, пиковая яркость которого составляет 2000 кд/м². Дисплей также имеет частоту обновления 120 Гц с технологией LTPO.',
        link:"https://kaspi.kz/shop/p/apple-iphone-14-128gb-goluboi-106363150/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/hbc/h02/64481569701918.jpg?format=preview-medium',
        rating:5,
        url:"https://t.me/zhakoooooooo12345/21",
        numberOfLikes:0
    }
    ,
    {
        name:'Samsung Galaxy A53 5G 6 ГБ/128 ГБ черный',
        description:'Любуйтесь яркими деталями на дисплее Super AMOLED 120 Гц в разрешении FHD+. ',
        link:"https://kaspi.kz/shop/p/samsung-galaxy-a53-5g-6-gb-128-gb-chernyi-104253225/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/h6f/h8d/64381860446238.jpg?format=preview-medium',
        rating:4.9,
        url:"https://t.me/zhakoooooooo12345/23",
        numberOfLikes:0
    }
    ,
    {
        name:'Samsung Galaxy A03 Core 2 ГБ/32 ГБ медный',
        description:'Больше дисплей, больше возможностей. Расширьте границы доступного с 6.5-дюймовым экраном с V-образным вырезом.',
        link:"https://kaspi.kz/shop/p/samsung-galaxy-a03-core-2-gb-32-gb-mednyi-105772530/?c=750000000",
        imgURL:'https://resources.cdn-kaspi.kz/img/m/p/h72/hf1/64525646626846.jpg?format=preview-medium',
        rating:4.5,
        url:"https://t.me/zhakoooooooo12345/22",
        numberOfLikes:0
    }

]